...
| Метрика | Что показывает | Лучше когда |
|---|---|---|
| `verticality_error` | Насколько линии фасада отклоняются от вертикали (в градусах) | Меньше, линии стоят ровнее |
| `parallelism_std` | Насколько линии параллельны друг другу (разброс углов) | Меньше, линии более параллельны |
| `vp_residual_px` | Среднее расстояние от линий до точки схода (в пикселях) | Меньше, точка схода найдена точнее |
| `vp_horizon_distance` | Как далеко точка схода от центра кадра (нормировано) | Больше = сильнее был наклон камеры |
